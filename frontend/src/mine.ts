...
// Minesweeper class (translated from Python)
class Minesweeper {
  height: number;
  width: number;
  mines: Set<string>;
  board: boolean[][];
  minesFound: Set<string>;
  ...
  won(flags: Set<string>): boolean {
    return this.mines.size === flags.size && 
           Array.from(this.mines).every(mine => flags.has(mine));
  }
}

// Sentence class (translated from Python)
class Sentence {
  cells: Set<string>;
  count: number;

  constructor(cells: Set<string>, count: number) {
    this.cells = new Set(cells);
    this.count = count;
  }

  knownMines(): Set<string> {
...
// MinesweeperAI class (translated from Python)
class MinesweeperAI {
  height: number;
  width: number;
  ...
  findNeighbours(cell: Cell): Set<string> {
    const [i, j] = cell;
