...
def block_user(request):
    try:
        ...
        if not target_id:
            return JsonResponse({'error': _('user_id is required')}, status=400)
        if str(user.id) == str(target_id):
            return JsonResponse({'error': _('Cannot block yourself')}, status=400)
        ...
        return JsonResponse({'success': True, 'message': _('You have blocked {username}').format(username=target.username)})
    except User.DoesNotExist:
